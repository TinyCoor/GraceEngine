...
KAPI void  _darray_destroy(void* array)
{
    ...
    kfree(array,total_size,MEMORY_TAG_DARRAY);
}
...
KAPI void  _darray_pop(void* array,void* dst)
{
    ...
    u64 addr = (u64)array;
    addr += (length-1) * stride;
    kcopy_memory(dst,(void*) addr,stride);
    _darray_field_set(array,DARRAY_LENGTH,length - 1);
}
...
KAPI void* _darray_pop_at(void* array,u64 index,void* dst)
{
    ...
    u64 stride = darray_stride(array);
    if(index >= length) {
        KERROR("Index out of the bounds of this array: Length: %i ,index: %index",length,index);
        return array;
    }

    u64 addr = (u64)array;
    kcopy_memory(dst,(void*)(addr +(index *stride)),stride);
