...
{
    kzero_memory(command_buffer,sizeof(vulkan_command_buffer));
    ...
    VK_CHECK(vkAllocateCommandBuffers(context->device.logical_device,
                             &allocate_info,&command_buffer->handle));
...
{
//    vkResetCommandBuffer(command_buffer->handle,)
    command_buffer->state= COMMAND_BUFFER_STATE_READY;
}
...
void vulkan_command_buffer_allocate_and_begin_single_use (
    ...
    vulkan_command_buffer* command_buffer)
{
    vulkan_command_buffer_allocate(context,pool,true,command_buffer);
    vulkan_command_buffer_begin(command_buffer,true,false,false);
}
